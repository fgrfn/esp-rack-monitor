...
Thresholds are deliberately separated by metric and device. Edit only the substitutions in `esp-rack-monitor.yaml`.
...
A value equal to a threshold enters that threshold level. With the default rack profile, 35 °C is therefore a warning and the critical alarm begins at 40 °C.
